...
exports.makeStatusPayload = (data) => {
  ...
  const age = date_1.calculateAge(data.birthDate.toDate());
  return {
    ...
    height: data.height,
    weight: data.weight,
    infected_discover_date: data.infectedDiscoverDate,
...
exports.convertToArray = (data) => {
  return [
    ...
    date_1.calculateAge(data.birthDate.toDate()),
    data.weight,
    data.height,
    data.gender,
    date_2.convertTZ(data.lastUpdatedAt.toDate()),
    data.address,
    data.district,
    data.prefecture,
    data.province,
